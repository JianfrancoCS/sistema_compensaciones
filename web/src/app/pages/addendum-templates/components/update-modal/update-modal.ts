...
@Component({
  selector: 'app-addendum-template-update-modal',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    CommonModule,
...
export class AddendumTemplateUpdateModal implements OnInit, OnChanges {
  @Input() visible: boolean = false;
  @Input() addendumTemplateId: string | null = null;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['visible'] && this.visible && this.addendumTemplateId) {
      this.addendumTemplateService.getAddendumTemplateForCommand(this.addendumTemplateId).subscribe(response => {
        if (response.success) {
          const template = response.data;
          this.templateForm.patchValue({
            name: template.name,
            addendumTypePublicId: template.addendumTypePublicId,
            statePublicId: template.statePublicId,
            templateContent: template.templateContent
          });
          this.initialContent.set(template.templateContent);
        }
      });
    }
  }
  ...
  updateTemplate() {
    if (this.templateForm.valid && this.addendumTemplateId) {
      const formValue = this.templateForm.value;
      this.addendumTemplateService.update(this.addendumTemplateId, {
        name: formValue.name!,
        ...
        error: (err) => {
          this.messageService.add({ severity: 'error', summary: 'Error', detail: err.message || 'Ocurrió un error al actualizar la plantilla.' });
        }
      });
    }
  }

...
export class AttendanceExitComponent implements OnInit {
    ...
    effect(() => {
      ...
      if (response) {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Éxito',
          detail: `Salida registrada: ${response.markedAt}`,
          life: 3000
        });

        this.attendanceForm.patchValue({ personDocumentNumber: '' });
  ...
  markAttendance(): void {
    ...
    const { personDocumentNumber, subsidiaryPublicId, markingReasonPublicId } = this.attendanceForm.value;
